...
type Attribute = schema.ServiceTypes.Attribute
...
let axisPred axis = <@ fun  (row1:Data) (row2:Data) -> row1.Entry = row2.Entry && (%(axisPred' axis)) row1 row2 @>
...
let xpath i data path = <@ seq{ for row in %data do 
                                for row' in (%(pathP2 data path)) row do
                                if (row.Parent = -1 && row.Entry = i) 
                                then yield row'.ID} @>;;


let printRow (row:Data) = printfn "ID:%d Name:%s Parent:%d Pre:%d Post:%d" row.ID row.Name row.Parent row.Pre row.Post;;
let printRows x = x |> Seq.iter printRow
let forceRows x = x |> Seq.iter (fun _ -> ())

let xpath' i data path = <@ query { for row in %data do 
